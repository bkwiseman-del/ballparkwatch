...
**Field monitor — installed-idle, stream-on-demand (the activation model).** A field-owned camera (a facility's fixed cam, or literally a spare phone dropped in a mount) stays **installed and idle**, not broadcasting. It **only starts streaming when there's a reason to**, on one of two triggers: **(a)** the field/facility creates and starts a game on that field (auto-start, with a **max session timeout** so a forgotten game doesn't stream an empty diamond for hours), or **(b)** someone **claims that field's matchup and starts broadcasting it** (a parent, a scorer, the league). When the game ends (or the timeout hits), the cam drops back to idle — or to the bare field feed if we want the "is it raining at the complex" ambient view. This is the cost-control mechanism *and* the UX: you never pay to stream an empty field, and the device is **immediately ready for the next game** without a human touching it.

> **Externally validated.** *CourtLive* (a tennis-club platform, same shape as Bandbox: phone broadcast + live score + player replays on Cloudflare WHIP) shipped exactly this as **"Court Monitor"** — a phone permanently installed courtside that auto-streams when a match is created, freeing volunteers from per-match setup. Independent convergence on the same design de-risks it. Their public blocker is **also ours**: Cloudflare WHIP has **no server-side recording yet**, so a fixed device is forced to *record locally then upload after the match*, which keeps the device busy and delays the replay — defeating the "immediately ready for the next game" promise. So the field-monitor pattern is the **strongest reason to solve server-side recording** (WebRTC→RTMP relay into Stream, or Cloudflare's eventual WHIP recording): a permanently-installed field cam should offload recording to the server, not tie up the mount. See the recording-architecture note in the build plan.
