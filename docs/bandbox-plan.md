...
- **Cloudflare Stream's WHIP/WHEP does NOT record — CONFIRMED (2026-07).** Empirically verified: multiple WHIP test broadcasts produced **0 recordings** account-wide; CF's own WebRTC docs list *"Recording is not yet supported (coming soon)"* with **no committed ETA**. Realtime SFU adapters don't help (only a JPEG-over-WebSocket adapter exists); **RealtimeKit** *does* record WebRTC→R2 but is a separate meetings product (a full re-platform, not a toggle). **What we shipped anyway:** phones publish **live** via WHIP→Stream (sub-second, unlimited viewers, ABR — this part is great), which is why the split matters:
  ...
  - **Recording (the replay):** must be solved separately. Options, ranked: **(1) local device recording** (hardware H.264 ~480p/24fps, single extra encode — the original plan's `MediaRecorder`-to-R2, works today, watch phone heat); **(2) WebRTC→RTMP relay → Stream** (a small self-hosted forwarder makes it RTMP, which Stream **does** record + ABR + deliver; ~$0.05–0.15/game compute; keeps our stack); **(3) migrate to RealtimeKit** (CF-native recording, heavy pivot); **(4) wait** for WHIP recording (no ETA). **Do #1 to unblock; #2 is the durable path.** This is *the* reason server-side recording matters for **field cams / the field-monitor pattern** (see the vision-market-wedge doc) — a permanently-installed device can't tie itself up recording+uploading between back-to-back games.
  - **DECISION (2026-07):** shipping **#1 now** (local recording, hardware H.264 ~480p/24fps — light second encode to keep the phone cool while WHIP delivers live). **To revisit:** spec/build **#2 (WebRTC→RTMP relay)** as the durable fix (also unlocks the field-monitor pattern); evaluate **#3 (RealtimeKit)** only if we want CF to own the whole media layer. **KEEP TABS:** watch the [CF Stream changelog](https://developers.cloudflare.com/stream/changelog/) + [WebRTC docs](https://developers.cloudflare.com/stream/webrtc-beta/) — when WHIP recording ships, delete the local encoder and it's free.
