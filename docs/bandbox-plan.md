...
- **Cloudflare Stream's WHIP/WHEP does NOT record — CONFIRMED (2026-07).** Empirically verified: multiple WHIP test broadcasts produced **0 recordings** account-wide; CF's own WebRTC docs list *"Recording is not yet supported (coming soon)"* with **no committed ETA**. Realtime SFU adapters don't help (only a JPEG-over-WebSocket adapter exists); **RealtimeKit** *does* record WebRTC→R2 but is a separate meetings product (a full re-platform, not a toggle). **What we shipped anyway:** phones publish **live** via WHIP→Stream (sub-second, unlimited viewers, ABR — this part is great), which is why the split matters:
  ...
  - **This gives a clean free/paid split (aligns cost with value):** **Free** = local recording (sub-second live + a modest 480p replay, $0 infra). **Paid** = the server-side recorder runs for their games → **full-quality replay**. Field cams → RTMP-direct to Stream (native recording, no bridge).
  - **DECISION (2026-07):** shipped **#1** as the free-tier replay + stopgap. **Build #2** (server-side WHEP recorder) as the durable, full-quality path = what paid accounts get. Live stays sub-second in every case. **KEEP TABS:** watch the [CF Stream changelog](https://developers.cloudflare.com/stream/changelog/) + [WebRTC docs](https://developers.cloudflare.com/stream/webrtc-beta/) — if WHIP recording ships, #2 collapses into a one-line Stream setting and we retire the bridge.
