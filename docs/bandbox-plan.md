...
- **YouTube/Facebook as our hosting is out** (field PA music = automated copyright strikes; a channel strike is existential). **BUT simulcast to a parent's own FB/YT is back on the table as a Phase-2 differentiator via "clean-audio simulcast":** mute the field mic and broadcast our own audio track instead — the crowd-noise bed + AI play-by-play we already generate — so the outbound RTMP carries **zero copyrighted material**, killing the strike risk *and* showcasing the AI booth ("every game goes out with an announcer"). Constraint is no longer legal but **infra + sync**: needs the RTMP-out media pipeline (CF Stream Live-Input → restream) + mixing the crowd/AI audio into the outbound track (WebRTC track-replace on the phone, or server-side) synced to the video. Opt-in, paid, phone's-own-channel; never the default.
- **Cloudflare Stream's WHIP/WHEP does NOT record — CONFIRMED (2026-07).** Empirically verified: multiple WHIP test broadcasts produced **0 recordings** account-wide; CF's own WebRTC docs list *"Recording is not yet supported (coming soon)"* with **no committed ETA**. Realtime SFU adapters don't help (only a JPEG-over-WebSocket adapter exists); **RealtimeKit** *does* record WebRTC→R2 but is a separate meetings product (a full re-platform, not a toggle). **What we shipped anyway:** phones publish **live** via WHIP→Stream (sub-second, unlimited viewers, ABR — this part is great), which is why the split matters:
  - **Live delivery:** Stream WHIP/WHEP (working, on-device validated).
  - **Recording (the replay):** solved separately from live. Options, ranked: **(1) local device recording** (hardware H.264 ~480p/24fps, a light second on-phone encode — works today, modest quality); **(2) server-side WHEP recorder** — a headless service *subscribes* to the live stream (as a WHEP viewer) and records the **full-quality** feed → the replay; **(3) migrate to RealtimeKit** (CF-native recording, heavy pivot); **(4) wait** for WHIP recording (no ETA).
  - **#2 is the durable path — and the key insight is it's a PASSIVE SUBSCRIBER, not a relay in the live path.** The phone still WHIPs full-quality straight to Cloudflare for **sub-second live, unchanged**; the recorder just watches that stream and captures it. So it **adds no live latency**, is **not a live-path SPOF** (if it hiccups, viewers keep watching — you only lose that replay), and the replay is **full quality** (no on-phone downscale, no extra phone heat). Cleanest form: recorder forwards **WHEP→RTMP into Stream**, so Cloudflare does the recording + ABR + replay hosting; we run only a thin bridge. Full spec: [bandbox-server-recorder-spec.md](bandbox-server-recorder-spec.md).
  - **DECISION (2026-07, REVISED): free tier is LIVE-ONLY — no video recording at all.** This dissolves the whole local-recording problem (phone heat, memory, quality, upload + storage cost) and makes cost track value. Cost principle: **free only ever gets what's free for us to produce.**
    - **Free:** live video (sub-second WHIP/WHEP; only cost is delivery, bounded by the viewer cap) **+ the DATA replay** — final/line score, box score, stats, play-by-play, all **projected from the event log at ~zero cost**. **No** AI recap, **no** AI commentary/TTS, **no** video replay (those all cost us per game).
    - **Paid:** + **AI recap** + **AI commentary** + **full-quality VIDEO replay** via the **server-side WHEP recorder (#2)** — a PASSIVE subscriber to the live stream (not a live-path relay): phone WHIPs full-quality for sub-second live *unchanged*, the recorder watches + captures it (WHEP→RTMP→Stream), so **no added live latency, no live SPOF, no phone heat, full quality**. Full spec: [bandbox-server-recorder-spec.md](bandbox-server-recorder-spec.md).
    - **Field cams:** RTMP-direct to Stream (native recording, no bridge).
    - **Consequence:** the phone's ONLY job is the live WHIP stream — local recording (WebCodecs/MediaRecorder/incremental upload) is **removed**. Video replay returns for **paid** when the server recorder is built.
  - **KEEP TABS:** watch the [CF Stream changelog](https://developers.cloudflare.com/stream/changelog/) + [WebRTC docs](https://developers.cloudflare.com/stream/webrtc-beta/) — if WHIP recording ships, the paid recorder collapses into a one-line Stream setting.
