-- Viewer needs player ids in the lineup (to project substitutions) and a roster
-- map (to resolve subbed-in players by id).
...
as $$
  select coalesce(
    jsonb_agg(
      jsonb_build_object('id', p.id, 'name', p.name, 'jersey', p.jersey_number, 'pos', le.position)
