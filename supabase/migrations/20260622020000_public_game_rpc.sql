-- Phase 1: no-account viewer read. A SECURITY DEFINER function returns only the
-- public-safe fields for one game, so anon can load the initial scorebug without
-- broad SELECT grants on game_state. Live updates ride Realtime Broadcast.
...
as $$
  select jsonb_build_object(
    ...
    'away', jsonb_build_object('name', away.name),
    'home', jsonb_build_object('name', home.name),
