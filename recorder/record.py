...
def main(whep_url: str, out_path: str) -> int:
    ...
    def try_link(pad):
        ...
        if media == "video":
            ...
            # Force CONSTANT frame rate AND explicit dimensions. The WebRTC feed arrives with
            # jittery frame timing; re-encoding those variable timestamps 1:1 yields a VFR mp4
            # that stalls on playback and drifts the play-sync. videorate duplicates/drops to a
            # steady 30fps. CRITICAL: also pin width/height — a framerate-only capsfilter left
            # the caps' resolution unfixed, so mp4mux wrote 0x0 dimensions. Chrome reads the size
            # from the H.264 stream and plays anyway, but Safari trusts the 0x0 container header
            # and renders BLACK. The broadcast canvas is always 1280x720, so scale+pin to that.
            rate = Gst.ElementFactory.make("videorate")
            scale = Gst.ElementFactory.make("videoscale")
            ratecaps = Gst.ElementFactory.make("capsfilter")
            ratecaps.set_property(
                "caps",
            ...
            link_chain(pad, [Gst.ElementFactory.make("queue"), depay, parse_in, dec, conv, rate, scale, ratecaps, enc, parse_out])
            log("linked video (H.264 re-encode, CFR 30fps, 1280x720)")
